...
import utils.kitti.object3d as object3d

def get_cls_type_mean_size(cls_type):
    cls_type_mean_size = {'Car': np.array([3.88311640418,1.62856739989,1.52563191462]),
    ...
    assert cls_type in cls_type_mean_size.keys()
    return cls_type_mean_size[cls_type]

cls_type_list = ['Car', 'Van', 'Truck', 'Pedestrian', 'Person_sitting', 'Cyclist', 'Tram', 'Misc']
cls_type_mean_size_arr = np.zeros((8, 3), dtype=np.float32) # size clustrs
for i in range(len(cls_type_list)):
    cls_type_mean_size_arr[i] = get_cls_type_mean_size(cls_type_list[i])

type2class={'Car':0, 'Van':1, 'Truck':2, 'Pedestrian':3, 'Person_sitting':4, 'Cyclist':5, 'Tram':6, 'Misc':7}
type2class_carpedcyc = {'Car': 0, 'Pedestrian': 1, 'Cyclist': 2}
